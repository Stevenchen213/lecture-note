...
6. 如果内容不足以形成完整章节，返回已有部分即可${pptHint}
...
export async function generateQuestions(transcripts, outline) {
  ...
  const result = await chat(
    [
      {
        role: 'system',
        content: `你是大学课程助教。根据课堂内容生成 15 道核心练习题，帮助期末复习巩固。

规则：
1. 从整个课程中挑选最核心的 15 个知识点出题，覆盖不同章节
2. 优先采用老师课上讲过的原始数据、数字、例子、定义、原话作为题目素材（老师很可能直接考原题/原数据）
3. 题型多样：选择题、简答题、判断题混合
4. 每道题包含中文题目(question)、英文题目(questionEn)、中文答案(answer)
5. 选择题选项用 A/B/C/D 标记，干扰项要合理，不能一眼看出是错的
6. 老师暗示会考的内容（如"this will be on the exam""重点""必考""记住"等）必须优先生成，排在最前面
7. 难度分层：约 60% 基础概念题 + 30% 应用题 + 10% 综合题
8. 15 道题覆盖不同知识点，避免同质化和重复
9. 按知识点重要性排序，考点暗示 > 核心概念 > 一般知识点
